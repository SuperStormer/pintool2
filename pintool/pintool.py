...
def get_args():
	...
	parser.add_argument(
		"-s", "--symbol", default="-", help="Symbol used as password placeholder"
	)
	parser.add_argument(
		"-e",
		"--expression",
...
def get_charset(charset_str, additional):
	charsets = {
		...
		"default2": string.ascii_lowercase + "_" + string.digits + "{}",
	...
	return "".join(charsets[name] for name in charset_str.split(",")) + "".join(
		additional
	)
...
async def detect_length(filename, arch, max_len, symbol="-", use_argv=False):
	with TemporaryDirectory() as tempdir:
		...
		for i in range(1, max_len + 1):
			...
			inscount = await pin(
				...
				password,
				use_argv=use_argv,
				out_file=f"{tempdir}/inscount{i}.out",
			)
			...
			print(
				f"{password} = with {i} characters difference {inscount - initial} instructions"
			)
...
async def solve(
	...
	with TemporaryDirectory() as tempdir:  # noqa: PLR1702
		...
		while True:
			...
			for i in r:
				...
				initial = await pin(
					filename,
					arch,
					get_password(password, symbol, i),
					additional_args,
					use_argv,
				...
				coros = [
					asyncio.create_task(
						helper(initial, get_password(password, char, i), char)
					)
					for char in charset
				]
